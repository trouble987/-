...
client.on("message", msg => {
    ...
    .addField("**➥ Link**", "[<a:kral:778787824018653205> Destek Sunucu](https://discord.gg/AjtzpzPQfv)")
...
client.on('message', message => {
    ...
    if(!b.includes(":warning: | `Ayarsız` Botlar Susturulacak.")) { işlem() }
    else {}

    function işlem() {

    if(b.length > 5) {
      message.channel.send(':warning: | `Ayarsız` Botlar Susturulacak.')
      aut.forEach(a => {
        message.channel.overwritePermissions(a, {
          "SEND_MESSAGES": false
        })
      })
      message.channel.send( ' | `Ayarsız` botlar susturuldu.')
    } else return;
    }
    })})})})
...
client.on('guildMemberAdd', member => {
 ...
 member.roles.add(member.guild.roles.cache.get(rol))
    ...
    .setFooter('Ayarsız Tercih Ettiğiniz İçin Teşekkür Ederiz.')
...
///////////////////////OtoCevap////////////////////////////
client.on("message", msg => {
  if (msg.content.toLowerCase() === "Ayarsız") {
...
log.send(new Discord.MessageEmbed()
.setTitle('Ayarsız - TAG Çıkarıldı.')
.setColor('#f6ff00')
